    ...
    public ConnectServiceRegistry(List<BindableService> services) {
        for (BindableService service : services) {
            ...
            for (ServerMethodDefinition<?, ?> methodDef : definition.getMethods()) {
                ...
                // Find the corresponding Java method on the service class. Built-in gRPC
                // services (health, reflection) use signatures that don't match the unary
                // 2-arg pattern below, so they're intentionally left unregistered here.
