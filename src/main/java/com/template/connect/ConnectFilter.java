...
public class ConnectFilter implements WebFilter {
                    ...
                    public void onCompleted() {
                        if (value == null) {
                            future.completeExceptionally(Status.INTERNAL
                                .withDescription("Unary RPC completed without a response value")
                                .asRuntimeException());
                        } else {
    ...
    private Mono<Void> writeProtoResponse(
        ...
        try (var stream = descriptor.streamResponse(message)) {
            // Protobuf marshallers back this stream with an in-memory buffer of the
            // already-serialized message, not real I/O — safe to read synchronously here.
